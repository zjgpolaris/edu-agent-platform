...
```http
POST /api/teacher/assignments/{assignment_id}/review
```
...
- 校验当前教师是否为该作业创建者；
- 更新 `assignment_submissions.score`、`status='graded'`、`teacher_feedback`、`reviewed_at`；
- 教师端提交详情中，`partial` / 待评阅提交显示分数和反馈输入框；
- 评阅后刷新作业详情与列表统计；
- 主观题知识点按整份作业分数粗粒度回流：低于 60 分写入错题本，高于等于 60 分尝试移除。

---

## 六、注意事项

- 简答题不自动判分，提交后状态为 `partial`，由教师人工评阅后转为 `graded`。
- 客观题错题回流使用逐题 `is_correct`；主观题回流使用教师评阅后的整份作业分数做粗粒度判断。
...
1. 将提交详情中的高频错题聚合成「本次作业讲评建议」；
2. AI 出题支持混合题型：同一批知识点按比例生成单选/判断/简答；
3. 把作业错题回流事件写入学习事件表，供成长报告展示来源。
